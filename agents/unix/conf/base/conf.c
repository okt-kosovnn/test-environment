             ...
             const char *user)
{
    ...
    rc = ta_user_gid_get(&user_gid, user);

    if (rc == 0)
        TE_SPRINTF(value, "%u", (unsigned int)user_gid);
             ...
             const char *user)
{
    ...
    rc = ta_user_uid_get(&user_uid, user);

    if (rc == 0)
        TE_SPRINTF(value, "%u", (unsigned int)user_uid);
